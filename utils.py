...
from metrics import bce_dice_loss, iou, dice_coef, accuracy_score
...
def train(args):
    ...
    train_set = BiONetDataset(args.train_data, 'chestxray', batchsize=args.batch_size, steps=args.steps, transforms=transforms)
    test_set = BiONetDataset(args.valid_data, 'chestxray', transforms=None)
...
def evaluate(args):
    test_set = BiONetDataset(args.valid_data, "chestxray")
    test_loader = DataLoader(dataset=test_set, batch_size=args.batch_size, shuffle=False, num_workers=0,
                             pin_memory=True)
    ...
    with torch.no_grad():
        for step, (x, y) in enumerate(tqdm(test_loader)):
            ...
            iou_score = iou(output, y)
            dice_score = dice_coef(output, y)
            acc_score = accuracy_score(output, y)
    ...
    if args.save_result:
        ...
        results = (results > 0.5).astype(np.float32)
